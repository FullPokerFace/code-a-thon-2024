        ...
        <VFlex className='navigation'>
            {
                navItems?.map(group => {
                    return <>
                        ...
                        {
                            group?.navItems?.map(nav => <NavItem key={nav?.label} isActive={nav?.label === active} {...nav} onClick={() => setActive(nav?.label)} />)
                        }
                    </>
                })
            }
...
const NavItem = ({ label, icon, onClick, isActive }: TNavItemProps) => {
    return (
        <HFlex>
            <a className={`navitem ${isActive ? 'active' : ''}`} onClick={onClick}>{icon} {label}</a>
        </HFlex>
    )
}
